...
describe('FeedPage - Debounced Masonry Layout Recalculation', () => {
  ...
  describe('Integration with virtualizer', () => {
    it('should debounce scroll margin updates for virtualizer', () => {
      const setScrollMargin = vi.fn()
      
      const updateScrollMargin = () => {
        // Simulate getBoundingClientRect + scrollY calculation
        const mockTop = 100 + Math.random() * 50
        setScrollMargin(mockTop)
      }
      
      const debouncedUpdate = debounce(updateScrollMargin, 150)

      // Simulate rapid layout changes that trigger ResizeObserver
      for (let i = 0; i < 15; i++) {
        debouncedUpdate()
        vi.advanceTimersByTime(50)
      }

      expect(setScrollMargin).not.toHaveBeenCalled()

      vi.advanceTimersByTime(150)

      // Only one scroll margin update
      expect(setScrollMargin).toHaveBeenCalledTimes(1)
    })

    it('should preserve correct scroll margin value after debouncing', () => {
      let currentScrollMargin = 0
      const setScrollMargin = vi.fn((value: number) => {
        currentScrollMargin = value
      })
      
      const updateScrollMargin = (top: number) => {
        setScrollMargin(top)
      }

      const debouncedUpdate = debounce(() => updateScrollMargin(250), 150)
      ...
      // Should have the correct final value
      expect(currentScrollMargin).toBe(250)
      expect(setScrollMargin).toHaveBeenCalledTimes(1)
      expect(setScrollMargin).toHaveBeenCalledWith(250)
    })
  })
})
